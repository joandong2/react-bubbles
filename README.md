...
Tokens are strings of text, that can be stored on the client-side using local storage, these are used to transmit information between users and server. mostly used for authentication, decisions, data transfer.

-   [ ] What steps can you take in your web apps to keep your data secure?

We can do validations in both client/browser and server side. Check for empty fields or hidden values, use complex passwords, add HTTPS protocols, and also filter whats being inserted/uploaded into the database/backend. Daily maintainance with server, use security tools available to keep an eye of any unwanted changes, or strange behaviours.
...
We used axios, an HTTP client that allows us to make requests from the browser. ff. methods that can be mapped to CRUD are .get, .post, .put, and .delete
